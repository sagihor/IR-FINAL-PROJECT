...
- IDX files are opened using mmap, so they are mapped into virtual memory.


How mmap works in this system:
- The operating system maps the IDX file into the process address space.
- Only the portions of the file that are actually accessed are loaded into RAM.
- This enables fast random access while keeping memory usage low.
